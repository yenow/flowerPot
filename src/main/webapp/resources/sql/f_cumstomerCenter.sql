...
insert into f_customerCenter (ccno,mno, title, content, regdate, moddate, category) 
values (ccno_seq.nextval, 3,'DOBBY is NOT free', 'DOBBY IS not FREE', sysdate, sysdate ,'2');
...
insert into f_customerCenter (ccno,mno, title, content, regdate, moddate, category) 
values (ccno_seq.nextval, 2,'DOBBY is NOT free', 'DOBBY WANNA FREE', sysdate, sysdate ,'2');
...
화분은 한 분 한 분의 인연을 소중하게 생각합니다. 감사합니다.(❀╹◡╹)', sysdate, sysdate ,'2');
...
delete  from  F_CUSTOMERCENTER where category='2';
delete  from  F_CUSTOMERCENTER where category='2';
select * from F_CUSTOMERCENTER where category ='2'
...
alter table f_2 add constraint f_ccno foreign key (ccno) references f_customerCenter(ccno);
*/
