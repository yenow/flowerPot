...
<!--                                                 <input type="text" class="form-control" name="" placeholder="직위" value="팀장">
 -->                                            </div>
                                    ...
                                    <button type="submit" class="btn btn-izone btn-flat pull-right" style="background-color: #212b52; color: white;">등록</button>
									...
									<div class="pull-right" style="text-align: right;">
										...
										<tbody>
											<c:forEach var="emp" items="${eList}">
												<tr>
													<td>${emp.empNo}</td>
													<td><c:if test="${empty emp.empId}">미발급</c:if>	<c:if test="${!empty emp.empId}">${emp.empId}</c:if></td>
													<td>${emp.empName}</td>
													<td><c:forEach var="dept" items="${emp.deptList}">${dept.deptName}</c:forEach></td>
													<td><c:forEach var="po" items="${emp.poList}">${po.poName}</c:forEach></td>
													<td></td>
													<td><button onclick="location='';">권한 변경</button></td>
													<td><%-- <button class=" pull-right" onclick="location='${pageContext.request.contextPath}/admin/employee/signUp'">아이디 발급</button> --%>
