...
<head>
<meta charset="utf-8" />
    <link rel="apple-touch-icon" sizes="76x76" href="../resources/assets2/img/apple-icon.png">
    ...
    <!-- CSS Files -->
    <link href="../resources/assets2/css/bootstrap.min.css" rel="stylesheet" />
    <link href="../resources/assets2/css/light-bootstrap-dashboard.css?v=2.0.0 " rel="stylesheet" />
    <!-- CSS Just for demo purpose, don't include it in your project -->
    <link href="${pageContext.request.contextPath}/resources/assets2/css/demo.css" rel="stylesheet" />
</head>
<body>
		...
		<div class="main-panel">
			...
			style="background-color: white; border: 2px solid #9765da; WIDTH: 60px; color: black; "> Q&A </button>
						...
						<div class="col-md-12">
							<div class="card">
								<div class="header">
									<h4 class="title" style="display:inline-block;">F A Q</h4>
			<button onclick="location='${pageContext.request.contextPath}/admin/customer/faq_write'" class="btn btn-izone btn-flat pull-right"
										...
										<tbody>
											<c:forEach var="faq" items="${fList}">
												<tr>
													...
													<td>${faq.title}</td>
													<td>
													<fmt:formatDate value="${faq.regDate}" pattern="yyyy년 MM월 dd일"/> 
													</td>
													<td>
													<c:if test="${!empty faq.modDate}">
													<fmt:formatDate value="${faq.modDate}" pattern="yyyy년 MM월 dd일"/> 
													</c:if>
													<c:if test="${empty faq.modDate}">
													미수정
													</c:if>
													</td>
													<td><button onclick="location='${pageContext.request.contextPath}/admin/customer/faq_content/${faq.ccno}'">내용</button>
													<button onclick="location='${pageContext.request.contextPath}/admin/customer/faq_del/${faq.ccno}'">삭제</button></td>
												</tr>
											</c:forEach>
										</tbody>
									</table>
										...
										<ul class="pagination justify-content-center">
											...
											<c:if test="${pc.prev}">
												<li class="page-item"><a class="page-link"
													href="<c:url value='/admin/customer/faq${pc.makeURI(pc.beginPage - 1)}' />"
											...
											<c:forEach var="pageNum" begin="${pc.beginPage}"
												...
												<li class="page-item"><a
													href="<c:url value='/admin/customer/faq${pc.makeURI(pageNum)}'/>"
											...
											<c:if test="${pc.next}">
												<li class="page-item"><a class="page-link"
													href="<c:url value='/admin/customer/faq${pc.makeURI(pc.endPage + 1)}' />"
...
$(function() {
	$('#searchBtn').click(function(){
		...
		location.href="${pageContext.request.contextPath}/admin/customer/faq?keyword="+keyword+"&condition="+condition;
	});
...
if(result==="writeSuccess"){
	alert('FAQ가 성공적으로 등록 되었습니다.');
}else if(result==="modSuccess"){
	alert('FAQ가 성공적으로 수정 되었습니다.');	
}else if(result==="delSuccess"){
	alert('FAQ가 성공적으로 삭제 되었습니다.');	
}
